...
def allow_react_check_commands():
    def predicate(ctx):
        if check_react_check_channel(ctx):
            return True
        else:
            raise errors.ReactCheckChannelCheckFail()

    return commands.check(predicate)


def check_react_check_channel(ctx):
    if ctx.guild is None:
        return True
    cr_channels = ctx.bot.config.get('comment_react_channels', [])
    return ctx.channel.id in cr_channels
